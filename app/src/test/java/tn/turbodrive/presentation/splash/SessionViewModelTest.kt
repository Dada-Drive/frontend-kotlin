...
import tn.turbodrive.data.network.AuthNavigationEvents
...
import tn.turbodrive.presentation.notifications.PushTokenRegistrar
import tn.turbodrive.presentation.session.SessionState

@OptIn(kotlinx.coroutines.ExperimentalCoroutinesApi::class)
class SessionViewModelTest {
    @Test
    fun noTokenUnauthenticated() =
        runTest {
    ...
    @Test
    fun getMeSuccessRiderAuthenticated() =
        runTest {
            Dispatchers.setMain(UnconfinedTestDispatcher())
            try {
                val tokenManager = mockk<TokenManager>(relaxed = true)
    ...
    @Test
    fun getMeNetworkErrorUsesCachedProfile() =
        runTest {
            Dispatchers.setMain(UnconfinedTestDispatcher())
            try {
                val tokenManager = mockk<TokenManager>(relaxed = true)
