...
class PushTokenRegistrar @Inject constructor(
    ...
    fun registerProvidedToken(scope: CoroutineScope, token: String) {
        if (token.isBlank()) return
        scope.launch {
            runCatching {
                notificationApiService.saveToken(NotificationTokenRequest(token = token))
    ...
    suspend fun unregisterToken() {
        runCatching { notificationApiService.removeToken() }
