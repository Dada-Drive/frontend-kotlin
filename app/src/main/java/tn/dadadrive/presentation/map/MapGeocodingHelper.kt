...
internal class MapGeocodingHelper(
    ...
    private val forwardCache: LinkedHashMap<String, List<AddressSearchHit>> =
        object : LinkedHashMap<String, List<AddressSearchHit>>(
            ...
            override fun removeEldestEntry(
                eldest: MutableMap.MutableEntry<String, List<AddressSearchHit>>
            ): Boolean = size > MapViewModelConstants.ADDRESS_SEARCH_CACHE_SIZE
        }

    private fun cacheKey(query: String): String =
    ...
    private fun rememberForwardGeocode(query: String, hits: List<AddressSearchHit>) {
        val key = cacheKey(query)
...
internal fun Address.toReadableString(): String? =
    getAddressLine(0)?.takeIf { it.isNotBlank() }?.let { formatAddressForDisplay(it) }
