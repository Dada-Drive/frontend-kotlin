...
internal object HereOfflineMapsManager {
    ...
    fun cachePath(context: Context): String =
        File(context.cacheDir, "here-sdk/cache").apply { mkdirs() }.absolutePath

    fun persistentMapPath(context: Context): String =
        File(context.filesDir, "here-sdk/persistent-map").apply { mkdirs() }.absolutePath
    ...
    private fun tryInvokeSetter(target: Any, methodName: String, arg: Any) {
        runCatching {
    ...
    private fun ensureTunisiaOfflineDownloadedIfSupported(engine: SDKNativeEngine) {
        ...
        runCatching {
            val fromEngineAsync = mapDownloaderClass.methods.firstOrNull { candidate ->
    ...
    private fun requestTunisiaDownload(mapDownloader: Any) {
        ...
        runCatching {
            val fn = mapDownloader.javaClass.methods.firstOrNull { candidate ->
    ...
    private fun handleTunisiaRegions(mapDownloader: Any, topRegions: List<Any>) {
        val all = flattenRegions(topRegions)
        val tunisia = all.firstOrNull { region ->
        ...
        val regionId = runCatching { tunisia.javaClass.getMethod("getRegionId").invoke(tunisia) }.getOrNull()
            ?: return
    ...
    private fun installedRegionIds(mapDownloader: Any): Set<String> {
        return runCatching {
            @Suppress("UNCHECKED_CAST")
            val installed = mapDownloader.javaClass.getMethod("getInstalledRegions")
    ...
    private fun startRegionDownload(mapDownloader: Any, regionId: Any) {
        val statusCbClass = runCatching {
            Class.forName("com.here.sdk.maploader.DownloadRegionsStatusListener")
        }.getOrNull() ?: return
        val statusCb = Proxy.newProxyInstance(
            ...
            when (method.name) {
                ...
                "onDownloadRegionsComplete" -> {
                    val error = args?.getOrNull(0)
                    if (error != null) Log.e(TAG, "Tunisia offline download failed: $error")
                    else Log.i(TAG, "Tunisia offline download completed.")
                }
    ...
    private fun regionString(target: Any, getter: String): String =
