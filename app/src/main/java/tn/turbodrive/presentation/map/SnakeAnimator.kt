...
internal class SnakeAnimator(
    ...
    private fun render(progressM: Double) {
        ...
        trailPolyline =
            makeSnakePolyline(trailCoords, TRAIL_COLOR, SNAKE_TRAIL_WIDTH)?.also {
                it.drawOrder = 5
        ...
        headPolyline =
            makeSnakePolyline(headCoords, HEAD_COLOR, SNAKE_HEAD_WIDTH)?.also {
                it.drawOrder = 6
                mapView.mapScene.addMapPolyline(it)
            }
    }
    ...
    private fun wrapForwardDistance(
        from: Double,
        to: Double,
        total: Double,
    ): Double {
