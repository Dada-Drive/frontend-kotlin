...
internal fun buildTeardropPinPath(
    pinW: Float,
    pinH: Float,
): Path {
    val cx = pinW / 2f
...
fun createPushPinBitmap(colorArgb: Int): Bitmap {
    val pinColor = colorArgb or 0xFF000000.toInt()
    val ballRadius = 12f * PUSH_PIN_MAP_SCALE
    val stemWidth = 2.2f * PUSH_PIN_MAP_SCALE
    val stemHeight = 18f * PUSH_PIN_MAP_SCALE
    val tipHeight = 5f * PUSH_PIN_MAP_SCALE
    val width = (ballRadius * 2f) + (4f * PUSH_PIN_MAP_SCALE)
    val height = (ballRadius * 2f) + stemHeight + tipHeight
    ...
    // Thin vertical stem + small pointed tip (flat style as reference screenshot).
    val stemTop = centerY + ballRadius - (1.2f * PUSH_PIN_MAP_SCALE)
    val stemBottom = stemTop + stemHeight
    val stemPath =
        Path().apply {
            moveTo(centerX - stemWidth / 2f, stemTop)
            lineTo(centerX + stemWidth / 2f, stemTop)
            lineTo(centerX + stemWidth / 2f, stemBottom)
            lineTo(centerX + (stemWidth * 0.75f), stemBottom)
            lineTo(centerX, stemBottom + tipHeight)
            lineTo(centerX - (stemWidth * 0.75f), stemBottom)
            lineTo(centerX - stemWidth / 2f, stemBottom)
            close()
        }
    aa.color = pinColor
    canvas.drawPath(stemPath, aa)

    // Pin head.
    aa.color = pinColor
    canvas.drawCircle(centerX, centerY, ballRadius, aa)

    // Inner white center point like the design sample.
    aa.color = 0xFFFFFFFF.toInt()
    canvas.drawCircle(centerX, centerY, ballRadius * 0.22f, aa)

    return bitmap
}

fun createLabeledPushPinBitmap(
    colorArgb: Int,
    label: String,
): Bitmap {
    val pin = createPushPinBitmap(colorArgb)
    if (label.isBlank()) return pin

    val textPaint =
        Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = 0xFFFFFFFF.toInt()
            textSize = 15f * PUSH_PIN_MAP_SCALE
    ...
    val horizPad = 10f * PUSH_PIN_MAP_SCALE
    val vertPad = 6f * PUSH_PIN_MAP_SCALE
    val badgeHeight = (fm.descent - fm.ascent) + vertPad * 2f
    val badgeWidth = textWidth + horizPad * 2f
    val corner = 9f * PUSH_PIN_MAP_SCALE
    val gap = 4f * PUSH_PIN_MAP_SCALE
    val sidePadding = 2f * PUSH_PIN_MAP_SCALE

    val outWidth = maxOf(pin.width.toFloat() + sidePadding * 2f, badgeWidth + sidePadding * 2f)
    val outHeight = badgeHeight + gap + pin.height
    val out = Bitmap.createBitmap(outWidth.toInt(), outHeight.toInt(), Bitmap.Config.ARGB_8888)
    val canvas = Canvas(out)

    val centerX = outWidth / 2f
    val badgeLeft = centerX - badgeWidth / 2f
    val badgeTop = 0f
    val badgeRight = centerX + badgeWidth / 2f
    val badgeBottom = badgeHeight
    ...
    val textBaseline = badgeTop + vertPad - fm.ascent
    canvas.drawText(label, centerX, textBaseline, textPaint)

    val pinLeft = centerX - pin.width / 2f
    val pinTop = badgeBottom + gap
    canvas.drawBitmap(pin, pinLeft, pinTop, null)
