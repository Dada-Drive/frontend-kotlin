...
internal object MapTrafficUtils {
    ...
    fun extractTrafficSpans(
        route: Route,
        logTag: String,
    ): List<PassengerTrafficSpan> {
    ...
    private fun resolveJamFactor(
        dynamicSpeedInfo: Any?,
        span: Any,
    ): Double? {
        ...
        if (dynamicSpeedInfo != null) {
            runCatching {
                val m =
                    dynamicSpeedInfo.javaClass.methods.firstOrNull {
        ...
        val trafficSpeed =
            callNumberGetter(span, "getTrafficSpeedInMetersPerSecond")
                ?: callNumberGetter(span, "trafficSpeedInMetersPerSecond")
        val baseSpeed =
            callNumberGetter(span, "getBaseSpeedInMetersPerSecond")
