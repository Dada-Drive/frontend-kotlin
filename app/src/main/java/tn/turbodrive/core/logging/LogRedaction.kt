package tn.turbodrive.core.logging
...
object LogRedaction {
    ...
    fun redactRequestBodyForLog(body: String): String {
        ...
        s =
            Regex("(\"token\"\\s*:\\s*\")([^\"]+)(\")", RegexOption.IGNORE_CASE)
                .replace(s) { "${it.groupValues[1]}[TOKEN]${it.groupValues[3]}" }
        return s
    }
    ...
    private fun redactDeviceId(value: String): String = if (value.length <= 8) "***" else "${value.take(8)}…"
